...
async function refreshGlobalToday(){
  ...
  const ranked=active.map(topBuyRank).filter(o=>o.eligible).sort((a,b)=>b.rank-a.rank).slice(0,3);
  if(sum)sum.innerHTML='<div><span>Pokémon activas</span><b>'+pokemon.length+'</b></div><div><span>Lorcana activas</span><b>'+lorcana.length+'</b></div><div><span>Pasan filtro compra</span><b>'+active.map(topBuyRank).filter(o=>o.eligible).length+'</b></div>';
  if(!ranked.length){
    const near=active.map(topBuyRank).filter(o=>(+o.x.price||0)>=o.gate.profile.min&&(+o.x.price||0)<=o.gate.profile.max).sort((a,b)=>b.rank-a.rank).slice(0,4);
    box.innerHTML='<div class="buyHead"><h3>Qué mirar hoy · Pokémon + Lorcana</h3><span>mercado combinado</span></div><div class="buyNone"><b>Ninguna compra supera todos los filtros hoy</b><span>Card Vault no fuerza una compra. Exige rango útil, margen absoluto, liquidez, frescura y evidencia suficiente.</span></div>'+
      (near.length?'<div class="nearMisses"><b>Las más cercanas (NO son compra)</b>'+near.map(o=>'<div><span>'+universeIcon(marketUniverseOf(o.x))+' '+o.x.name+' · '+money(o.x.price,o.x.currency||"EUR")+'</span><small>'+[...(o.historyOk?[]:["historial insuficiente"]),...(o.pokemonEvidence?[]:["fuentes insuficientes"]),...o.gate.reasons,...o.d.reasons].slice(0,2).join(" · ")+'</small></div>').join("")+'</div>':'');
    return;
  }
  box.innerHTML='<div class="buyHead"><h3>Top 3 global · Pokémon + Lorcana</h3><span>solo candidatas que pasan todos los gates</span></div>'+
    ranked.map((o,i)=>{const x=o.x,z=o.d.zone;return '<article class="buyPick"><div class="buyRank">#'+(i+1)+'</div>'+(x.image?'<img src="'+x.image+'" alt="">':'<div class="buyNoImg">🃏</div>')+'<div class="buyBody"><b>'+universeIcon(marketUniverseOf(x))+' '+x.name+'</b><small>'+[x.set,x.rarity,x.finish].filter(Boolean).join(" · ")+'</small><div class="buyReason">Pasa precio + margen + liquidez + frescura + evidencia</div><div class="buyMetrics"><span>Precio <strong>'+money(x.price,x.currency||"EUR")+'</strong></span><span>Potencial <strong>+'+money(o.gate.upside,x.currency||"EUR")+'</strong></span><span>Escenario <strong>'+money(x.scenario12,x.currency||"EUR")+'</strong></span><span>Zona máx. <strong>'+money(z.high,x.currency||"EUR")+'</strong></span><span>Conv <strong>'+o.d.conv+'/100</strong></span><span>Liq <strong>'+o.d.liq+'/100</strong></span></div></div></article>'}).join("")+
    '<small class="buyFoot">Filtro cuantitativo y conservador. No garantiza beneficio ni sustituye comprobar el anuncio real, estado, envío y vendedor antes de comprar.</small>';
}
...
function renderProductCompletion(){
  ...
  box.innerHTML='<h3>Estado de la aplicación V64</h3><div class="readinessScore '+(p.score===100?"complete":"")+'">'+p.score+'%</div>'+
...
function renderReadiness(){
  ...
  const blocks=readinessBlockers();box.innerHTML='<h3>Evidencia para uso real</h3><div class="readinessScore">'+r.score+'%</div>'+r.checks.map(c=>'<div class="qaRow"><span>'+c.label+'</span><b class="'+(c.ok?'ok':'warn')+'">'+(c.ok?'OK':'Pendiente')+'</b></div>').join("")+(blocks.length?'<div class="nextBlocker"><b>Siguiente bloqueo</b><span>'+blocks[0]+'</span></div>':'<div class="certDone">✅ Lista para el hito V64.</div>')+'<small>Este porcentaje mide evidencia real; no mide si el desarrollo de la app está terminado.</small>';
}
...
function renderValuationAudit(){
  ...
  const rows=[
    ...
    ["Top global Pokémon + Lorcana","Sí","Filtro combinado V64"],
    ["Pregrado PSA fotográfico","Sí","Función propia; no sustituye PSA"],
