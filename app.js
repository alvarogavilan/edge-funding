...
function productCompletionScore(){
  ...
  const checks=[
    ...
    {label:"Filtro económico de compra activo",ok:profile},
...
function renderProductCompletion(){
  ...
  box.innerHTML='<h3>Estado de la aplicación V62</h3><div class="readinessScore '+(p.score===100?"complete":"")+'">'+p.score+'%</div>'+
...
function renderProductCompletion(){
  ...
  box.innerHTML='<h3>Estado de la aplicación V62</h3><div class="readinessScore '+(t.score===100?"complete":"")+'">'+t.score+'%</div>'+
...
function renderReadiness(){
  ...
  const blocks=readinessBlockers();box.innerHTML='<h3>Evidencia para uso real</h3><div class="readinessScore">'+r.score+'%</div>'+r.checks.map(c=>'<div class="qaRow"><span>'+c.label+'</span><b class="'+(c.ok?'ok':'warn')+'">'+(c.ok?'OK':'Pendiente')+'</b></div>').join("")+(blocks.length?'<div class="nextBlocker"><b>Siguiente bloqueo</b><span>'+blocks[0]+'</span></div>':'<div class="certDone">✅ Lista para el hito V62.</div>')+'<small>Este porcentaje mide evidencia real; no mide si el desarrollo de la app está terminado.</small>';
}
