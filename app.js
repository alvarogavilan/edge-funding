...
document.querySelector("#scanMarket").onclick=runMarketScan;renderMarketScan();
...
function buildMarketSignal(c){
  ...
  return {id:c.id,name:c.name,set:c.set?.name||"",image:c.image?c.image+"/low.webp":"",price:trend,low,avg1:a1,avg7:a7,avg30:a30,momentum1:m1,momentum7:m7,discount,volatility:vol,global,tcgplayer:tcgplayerMarket(c),score,risk,scenario12,rarity:c.rarity||"",updated:cm.updated||null,analysts};
}
...
async function fetchMarketUniverse(){
  ...
  for(const name of MARKET_NAMES){
    try{
      ...
      for(const c of a.slice(-8)){if(!seen.has(c.id)){seen.add(c.id);briefs.push(c)}}
    }catch{}
  }
  ...
  briefs=briefs.slice(0,72);
  const full=await mapLimit(briefs,4,async b=>tcgdexCard("en",b.id));
  return full.filter(Boolean);
}
function median(a){if(!a.length)return null;let x=[...a].sort((a,b)=>a-b),m=Math.floor(x.length/2);return x.length%2?x[m]:(x[m-1]+x[m])/2}
function renderMarketIndex(rows){
  const box=document.querySelector("#marketIndex"),sectors=document.querySelector("#sectorGrid");if(!box||!sectors)return;
  if(!rows.length){box.innerHTML="";sectors.innerHTML="";return}
  const medScore=median(rows.map(x=>x.score))||0,medMomentum=median(rows.map(x=>x.momentum7*100))||0,breadth=rows.filter(x=>x.momentum7>0).length/rows.length*100,highRisk=rows.filter(x=>x.risk==="Alto").length/rows.length*100;
  box.innerHTML="<div><span>Índice Card Vault</span><b>"+Math.round(medScore)+"/100</b></div><div><span>Momentum mediano</span><b class=\""+(medMomentum>=0?"up":"down")+"\">"+(medMomentum>=0?"+":"")+medMomentum.toFixed(1)+"%</b></div><div><span>Amplitud positiva</span><b>"+breadth.toFixed(0)+"%</b></div><div><span>Riesgo alto</span><b>"+highRisk.toFixed(0)+"%</b></div>";
  const groups={};for(const r of rows){let k=r.name||"Otros";(groups[k]??=[]).push(r)}
  const top=Object.entries(groups).map(([name,g])=>({name,count:g.length,score:median(g.map(x=>x.score))||0,momentum:median(g.map(x=>x.momentum7*100))||0})).sort((a,b)=>b.score-a.score).slice(0,6);
  sectors.innerHTML=top.map(s=>"<div class=\"sector\"><b>"+s.name+"</b><span>"+s.count+" cartas · señal "+Math.round(s.score)+"</span><small class=\""+(s.momentum>=0?"up":"down")+"\">"+(s.momentum>=0?"+":"")+s.momentum.toFixed(1)+"% 7/30d</small></div>").join("");
}
function renderMarketScan(){
  const box=document.querySelector("#marketLeaders");if(!box)return;
  const rows=[...(state.marketScan||[])].sort((a,b)=>b.score-a.score);renderMarketIndex(rows);
  if(!rows.length){box.innerHTML='<div class="empty">Pulsa «Escanear mercado» para crear el primer radar cuantitativo.</div>';return}
  box.innerHTML='<div class="marketGrid">'+rows.slice(0,12).map((x,i)=>'<article class="marketAsset">'+(x.image?'<img src="'+x.image+'" alt="">':'')+'<div class="assetBody"><div class="assetTop"><b>#'+(i+1)+' '+x.name+'</b><span class="signal '+(x.score>=75?'hot':x.score>=60?'warm':'')+'">'+x.score+'</span></div><small>'+x.set+(x.rarity?' · '+x.rarity:'')+'</small><div class="assetMetrics"><span>Mercado <b>'+euro(x.price)+'</b></span><span>Riesgo <b>'+x.risk+'</b></span><span>1d <b class="'+(x.momentum1>=0?'up':'down')+'">'+(x.momentum1>=0?'+':'')+(x.momentum1*100).toFixed(1)+'%</b></span><span>7/30d <b class="'+(x.momentum7>=0?'up':'down')+'">'+(x.momentum7>=0?'+':'')+(x.momentum7*100).toFixed(1)+'%</b></span></div><div class="analystStrip"><span>Mom '+(x.analysts?.momentum??0)+'</span><span>Valor '+(x.analysts?.value??0)+'</span><span>Estab '+(x.analysts?.stability??0)+'</span><span>Global '+(x.analysts?.global??0)+'</span><span>Datos '+(x.analysts?.data??0)+'</span></div><div class="thesis">'+(x.discount>=.12?'Descuento visible frente a tendencia. ':'')+(x.global?'Precio también presente en TCGplayer. ':'')+'Escenario 12m: '+euro(x.scenario12)+' (no es predicción).</div></div></article>').join("")+'</div>';
}
async function runMarketScan(){
  const btn=document.querySelector("#scanMarket"),st=document.querySelector("#marketScanState");btn.disabled=true;st.textContent="Construyendo universo…";
  try{
    const cards=await fetchMarketUniverse();st.textContent="Analizando "+cards.length+" cartas…";
    const signals=cards.map(buildMarketSignal).filter(Boolean);
    state.marketScan=signals;state.marketScanAt=new Date().toISOString();save();renderMarketScan();
...
function blobToDataURL(blob){return new Promise((ok,no)=>{let r=new FileReader();r.onload=()=>ok(r.result);r.onerror=()=>no(r.error);r.readAsDataURL(blob)})}function dataURLToBlob(s){let [h,d]=s.split(","),mime=(h.match(/:(.*?);/)||[])[1]||"image/jpeg",bin=atob(d),a=new Uint8Array(bin.length);for(let i=0;i<bin.length;i++)a[i]=bin.charCodeAt(i);return new Blob([a],{type:mime})}function renderQA(){
  ...
  const pending=state.cards.filter(c=>c.draft).length,photos=state.cards.filter(c=>c.photoKey).length,scan=(state.marketScan||[]).length,last=state.marketScanAt?new Date(state.marketScanAt).toLocaleString("es-ES"):"Nunca";
  const checks=[["Build","V21","ok"],["Colección",state.cards.length+" fichas","ok"],["Fotos locales",photos+" guardadas",photos?"ok":"warn"],["Pendientes OCR",String(pending),pending?"warn":"ok"],["Market Lab",scan+" analizadas",scan?"ok":"warn"],["Último escaneo",last,scan?"ok":"warn"]];
