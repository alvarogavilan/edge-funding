...
function topBuyRank(x){
  ...
  const eligible=gate.ok&&historyOk&&(+x.price||0)>0&&d.conv>=76&&d.liq>=65&&x.risk!=="Alto"&&fresh<=14&&zone&&x.price<=zone.high&&data>=60;
  ...
  return {x,d,fresh,data,eligible,rank,gate,historyOk};
}
...
function renderProductCompletion(){
  ...
  box.innerHTML='<h3>Estado de la aplicación V63</h3><div class="readinessScore '+(p.score===100?"complete":"")+'">'+p.score+'%</div>'+
...
function renderReadiness(){
  ...
  const blocks=readinessBlockers();box.innerHTML='<h3>Evidencia para uso real</h3><div class="readinessScore">'+r.score+'%</div>'+r.checks.map(c=>'<div class="qaRow"><span>'+c.label+'</span><b class="'+(c.ok?'ok':'warn')+'">'+(c.ok?'OK':'Pendiente')+'</b></div>').join("")+(blocks.length?'<div class="nextBlocker"><b>Siguiente bloqueo</b><span>'+blocks[0]+'</span></div>':'<div class="certDone">✅ Lista para el hito V63.</div>')+'<small>Este porcentaje mide evidencia real; no mide si el desarrollo de la app está terminado.</small>';
}
