...
function renderRadar(){
  const u=currentRadarUniverse(),auto=[...(state.marketScan||[])].filter(x=>marketUniverseOf(x)===u&&(+x.price||0)>0&&(+x.price||0)<=radarLimit).sort((a,b)=>(+b.score||0)-(+a.score||0));
...
let catalogPage=0,catalogPageSize=60,catalogVisibleRows=[];
...
function renderCatalogDetail(row){
  ...
  box.innerHTML='<div class="detailTop">'+(row.image?'<img src="'+row.image+'" alt="">':'<div class="catalogNoImg">🃏</div>')+
...
function renderTechnicalCompletion(){
  ...
  box.innerHTML='<h3>Completitud técnica V58</h3><div class="readinessScore '+(t.score===100?"complete":"")+'">'+t.score+'%</div>'+
...
function renderCertification(){
  ...
  box.innerHTML=head+(blocks.length?'<div class="blockers"><b>Para llegar al 100% de datos reales:</b>'+blocks.map(x=>'<div>• '+x+'</div>').join("")+'</div>':'<div class="certDone">✅ Todos los criterios de V58 están superados.</div>');
}
