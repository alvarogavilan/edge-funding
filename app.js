...
function buildMarketSignal(c){
  ...
  const global=!!tcgplayerMarket(c);
  const momentumPts=clamp((m1*35+m7*65)*100, -20, 35);
  const discountPts=clamp(discount*45,0,25);
  const stabilityPts=clamp((0.22-vol)*70,0,15);
  const globalPts=global?10:0;
  const dataPts=[a1,a7,a30,low,trend].filter(Boolean).length>=4?15:8;
  const score=Math.round(clamp(35+momentumPts+discountPts+stabilityPts+globalPts+dataPts,0,100));
  const risk=vol<0.08?"Bajo":vol<0.18?"Medio":"Alto";
  const scenario12=trend*(1+clamp(m7*3,-0.25,0.35));
  return {id:c.id,name:c.name,set:c.set?.name||"",image:c.image?c.image+"/low.webp":"",price:trend,low,avg1:a1,avg7:a7,avg30:a30,momentum1:m1,momentum7:m7,discount,volatility:vol,global,tcgplayer:tcgplayerMarket(c),score,risk,scenario12,rarity:c.rarity||"",updated:cm.updated||null};
}
...
async function runMarketScan(){
  ...
  try{
    ...
    st.textContent=signals.length+" cartas analizadas · "+new Date().toLocaleTimeString("es-ES",{hour:"2-digit",minute:"2-digit"});
