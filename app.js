...
function topBuyRank(x){
  const d=decisionFor(x),fresh=ageDays(x.updated),data=+x.analysts?.data||0,zone=d.zone,gate=investmentGate(x);
  const eligible=gate.ok&&(+x.price||0)>0&&d.conv>=76&&d.liq>=65&&x.risk!=="Alto"&&fresh<=14&&zone&&x.price<=zone.high&&data>=60;
  ...
  return {x,d,fresh,data,eligible,rank,gate};
}
...
function renderTechnicalCompletion(){
  ...
  box.innerHTML='<h3>Completitud técnica V61</h3><div class="readinessScore '+(t.score===100?"complete":"")+'">'+t.score+'%</div>'+
...
function renderCertification(){
  ...
  box.innerHTML=head+(blocks.length?'<div class="blockers"><b>Para llegar al 100% de datos reales:</b>'+blocks.map(x=>'<div>• '+x+'</div>').join("")+'</div>':'<div class="certDone">✅ Todos los criterios de V61 están superados.</div>');
}
